<template>
	<div>
		<div v-if="canRegisterProtocol || hasInstallPromptEvent">
			...
			>
				Add Nexus Lounge to Home screen
			</button>
			<button
			...
			>
				Open irc:// URLs with Nexus Lounge
			</button>
		</div>
		...
		<div v-if="!store.state.serverConfiguration?.public">
			...
			<label class="opt">
				...
				<input
					...
					name="awayMessage"
					class="input"
					placeholder="Away message if Nexus Lounge is not open"
				/>
			</label>
		</div>
	</div>
...
export default defineComponent({
	...
	setup() {
		...
		const registerProtocol = () => {
			...
			window.navigator.registerProtocolHandler("irc", uri, "Nexus Lounge");
			// @ts-expect-error
			window.navigator.registerProtocolHandler("ircs", uri, "Nexus Lounge");
		};
