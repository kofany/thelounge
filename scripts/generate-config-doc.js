...
//
// ```sh
// npm run generate:config:doc ../nexuslounge.github.io/
// ```
...
if (!docRoot) {
	log.error("Missing DOC_REPO_PATH. Pass the path to the cloned `nexuslounge.github.io` repo.");
	process.exit(1);
}
...
const extractedDoc = configContent
	.replace(/https:\/\/nexuslounge\.chat\/docs/g, "/docs") // make links relative
	.split("\n")
	.reduce((acc, line) => {
		line = line.trim();
...
https://github.com/nexuslounge/nexuslounge/blob/master/scripts/generate-config-doc.js`;
