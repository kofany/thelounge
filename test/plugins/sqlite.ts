...
describe("SQLite migrations", function () {
    ...
    function serialize_run(stmt: string, ...params: unknown[]): Promise<void> {
        try {
    ...
    it("has a down migration for every migration", function () {
        ...
        const rollbackVersions = rollbacks.map((r) => r.version);
        expect(migrationVersions).to.have.ordered.members(rollbackVersions);
    });
    ...
    it("has working up-migrations", async function () {
        ...
        const allStmts = to_execute.map((m) => m.stmts).flat();
...
describe("SQLite unit tests", function () {
    ...
    it("deletes messages when asked to", async function () {
        ...
        const deletedMsg = messages.find((m) => m.text === "msg 13");
        expect(deletedMsg).to.be.undefined; // oldest gets deleted first
        ...
        expect(messageTexts).to.have.ordered.members(["msg 1", "msg 0"]);
    });
    ...
    it("deletes only the types it should", async function () {
        ...
        expect(messageTypes).to.have.ordered.members([
            MessageType.MESSAGE,
            MessageType.PART,
            MessageType.AWAY,
        ]);
        ...
        messages = await store.getMessages(net, chan, () => id++);
        const remainingTypes = messages.map((m) => m.type);
        expect(remainingTypes).to.have.ordered.members([MessageType.AWAY]);
    });
});
