...
function initializeIrssiClient(
	...
	(socket as any).on("network:list", async (callback: any) => {
		log.info(`[network:list] Request from ${socket.id}`);

		try {
			const networks = await client.networkManager.listNetworks();
			log.info(`[network:list] Found ${networks.length} networks`);

			if (typeof callback === "function") {
				callback({success: true, networks});
			}
		} catch (error) {
			log.error(`[network:list] Error: ${error}`);
	...
	(socket as any).on("network:add", async (data: any, callback: any) => {
		log.info(`[network:add] Request from ${socket.id}:`, data);

		if (!_.isPlainObject(data)) {
			log.error(`[network:add] Invalid data type`);
		...
		try {
			...
			const executeCommand = async (cmd: string) => {
				...
				await client.irssiConnection.executeCommand(cmd);
			};

			await client.networkManager.addNetwork(data.network, executeCommand);
			log.info(`[network:add] Network ${data.network.name} added successfully`);

			if (typeof callback === "function") {
				callback({success: true});
			}
		} catch (error) {
			log.error(`[network:add] Error: ${error}`);
	...
	(socket as any).on("network:connect", async (data: any, callback: any) => {
		log.info(`[network:connect] Request from ${socket.id}:`, data);

		if (!_.isPlainObject(data) || !data.networkName) {
			log.error(`[network:connect] Invalid data or missing networkName`);

			if (typeof callback === "function") {
				callback({success: false, error: "Invalid data or missing networkName"});
			}
		...
		try {
			...
			await client.networkManager.connectToNetwork(data.networkName, executeCommand);
			log.info(`[network:connect] Connected to network ${data.networkName}`);
	...
	(socket as any).on("network:disconnect", async (data: any, callback: any) => {
		log.info(`[network:disconnect] Request from ${socket.id}:`, data);

		if (!_.isPlainObject(data) || !data.networkTag) {
			log.error(`[network:disconnect] Invalid data or missing networkTag`);

			if (typeof callback === "function") {
				callback({success: false, error: "Invalid data or missing networkTag"});
			}
		...
		try {
			...
			await client.networkManager.disconnectFromNetwork(data.networkTag, executeCommand);
			log.info(`[network:disconnect] Disconnected from network ${data.networkTag}`);
	...
	(socket as any).on("network:remove", async (data: any, callback: any) => {
		log.info(`[network:remove] Request from ${socket.id}:`, data);

		if (!_.isPlainObject(data) || !data.networkName) {
			log.error(`[network:remove] Invalid data or missing networkName`);
		...
		try {
			...
			await client.networkManager.removeNetwork(data.networkName, executeCommand);
			log.info(`[network:remove] Network ${data.networkName} removed`);
