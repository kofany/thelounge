...
class SqliteMessageStorage implements SearchableMessageStorage {
    ...
    close(): Promise<void> {
        ...
        try {
            ...
            return Promise.reject(new Error(`Failed to close sqlite database: ${message}`));
        }
    }
    ...
    private flushBatch(): void {
        ...
        try {
            ...
            const message = err instanceof Error ? err.message : String(err);
            log.error(`Failed to flush message batch: ${message}`);
    ...
    private scheduleBatchFlush(): void {
        ...
        this.batchTimer = setTimeout(() => {
            ...
            try {
                ...
                const message = err instanceof Error ? err.message : String(err);
                log.error(`Batch flush error: ${message}`);
            }
        }, this.batchTimeout);
    }
